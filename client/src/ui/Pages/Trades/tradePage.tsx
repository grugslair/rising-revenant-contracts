...
export const TradesPage: React.FC<TradesPageProps> = ({ setMenuState }) => {
  ...
  return (
    <div className="game-page-container" >
      ...
      {shopState === ShopState.REINFORCES && <PageTitleElement imagePosition={ImagesPosition.RIGHT} name={"TRADES"} rightPicture={"close_icon.png"} rightImageFunction={() => { closePage() }} htmlContentsRight={<ReinforcementCountElement style={{marginRight:"8%"}}/>} styleContainerRight={{display:"flex", justifyContent:"flex-end" , alignItems:"center"}}/>}
      {shopState === ShopState.SELL_REINF && <PageTitleElement imagePosition={ImagesPosition.LEFT} name={"TRADES"} leftPicture={"left-arrow.png"} leftImageFunction={() => { setShopState(ShopState.REINFORCES) }} htmlContentsRight={<ReinforcementCountElement style={{marginRight:"8%"}}/>} styleContainerRight={{display:"flex", justifyContent:"flex-end" , alignItems:"center"}}/>}

      {shopState === ShopState.OUTPOST && <PageTitleElement imagePosition={ImagesPosition.RIGHT} name={"TRADES"} rightPicture={"close_icon.png"} rightImageFunction={() => { closePage() }} />}
      {shopState === ShopState.SELL_POST && <PageTitleElement imagePosition={ImagesPosition.LEFT} name={"TRADES"} leftPicture={"left-arrow.png"} leftImageFunction={() => { setShopState(ShopState.OUTPOST) }}/>}
...
//     func to call on dojo({ game_id, item_id: 2 })
