...
export const ReinforcementListingElement = ({ trade, showOwn, showOthers, playerWalletAmount, clientComponents, account, revoke_trade_reinf, purchase_trade_reinf, modify_trade_reinf }: {
    trade: Maybe<World__Entity> | undefined,
    showOwn: boolean | undefined, showOthers: boolean | undefined,
    playerWalletAmount: number | undefined
    clientComponents: any,
    account: any,
    revoke_trade_reinf: any,
    purchase_trade_reinf: any,
    modify_trade_reinf: any,
    }) => {

    const trade_model = trade?.models?.find((m) => m?.__typename == 'Trade') as Trade;

    const [numberValue, setNumberValue] = useState<number | null>(1);
    const [interactedWith, setInteractedWith] = useState<boolean>(false);
    ...
    return (
        <ClickWrapper className="reinforcement-sale-element-container" style={{ filter: playerWalletAmount! < Number(BigInt(trade_model?.price)) ? 'grayscale(70%)' : '' }}>
                    ...
                    {account.address === trade_model?.seller ?
                        <Tooltip title={
                            ...
                            <Box component={"span"} p={1} width={200} height={200} >
                                <h2 style={{ textAlign: "center", marginTop: "1px", fontFamily: "OL", fontWeight: "100" }}>Change Price</h2>
            ...
            {clientGameDate.guest ? <div className="reinf-grid-buy-button center-via-flex" style={{ filter: "brightness(70%) grayscale(70%)" }}>  <h2 className="test-h2">BUY NOW</h2></div> :
                <>
                    {account.address === trade_model?.seller ?
                        <div className="reinf-grid-buy-button center-via-flex pointer" onClick={() => revokeTrade()} ><h2 className="test-h2">REVOKE</h2></div>
                        :
                        <div className="reinf-grid-buy-button center-via-flex pointer" onClick={() => buyTrade()}><h2 className="test-h2">BUY NOW</h2></div>}
                </>}
